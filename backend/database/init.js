...
async function checkAndAddMissingColumns() {
    try {
        ...
        // Check for multiplier column in lobby_players table
        const multiplierExists = await checkColumnExists('lobby_players', 'multiplier');
        if (!multiplierExists) {
            console.log('➕ Adding multiplier column to lobby_players table...');
            await query(`
                ALTER TABLE lobby_players 
                ADD COLUMN multiplier INTEGER DEFAULT 1
            `);
            console.log('✅ Added multiplier column to lobby_players table');
        }
        
        // Check for other missing columns in hall_of_fame table
        const usernameExists = await checkColumnExists('hall_of_fame', 'username');
        if (!usernameExists) {
            console.log('➕ Adding username column to hall_of_fame table...');
            await query(`
                ALTER TABLE hall_of_fame 
                ADD COLUMN username VARCHAR(16) NOT NULL DEFAULT 'Unknown'
            `);
            console.log('✅ Added username column to hall_of_fame table');
        }

        const characterExists = await checkColumnExists('hall_of_fame', 'character');
        if (!characterExists) {
            console.log('➕ Adding character column to hall_of_fame table...');
            await query(`
                ALTER TABLE hall_of_fame 
                ADD COLUMN character VARCHAR(4) NOT NULL DEFAULT '🎮'
            `);
            console.log('✅ Added character column to hall_of_fame table');
        }

        const questionsExists = await checkColumnExists('hall_of_fame', 'questions');
        if (!questionsExists) {
            console.log('➕ Adding questions column to hall_of_fame table...');
            await query(`
                ALTER TABLE hall_of_fame 
                ADD COLUMN questions INTEGER NOT NULL DEFAULT 0
            `);
            console.log('✅ Added questions column to hall_of_fame table');
        }

        console.log('✅ Column check completed');
    } catch (error) {
        console.error('❌ Error checking/adding missing columns:', error);
        throw error;
    }
}
