...
describe('SocketService', () => {
  ...
  beforeEach(() => {
    ...
    mockGameService = new GameService(mockIo) as jest.Mocked<GameService>;
    mockUserRepository = new UserRepository() as jest.Mocked<UserRepository>;
    mockRequestLogger = RequestLogger as jest.Mocked<typeof RequestLogger>;
    ...
    // Mock the broadcastToLobby method
    jest.spyOn(socketService, 'broadcastToLobby').mockImplementation(() => {});

    // Mock LobbyService static method properly
    jest.spyOn(LobbyService, 'isValidLobbyCode').mockImplementation(() => true);
  });
  ...
  describe('Lobby Management', () => {
    describe('handleJoinLobby', () => {
      it('should successfully join a valid lobby', async () => {
        ...
        jest.spyOn(LobbyService, 'isValidLobbyCode').mockReturnValue(true);
      ...
      it('should reject invalid lobby code format', async () => {
        ...
        jest.spyOn(LobbyService, 'isValidLobbyCode').mockReturnValue(false);
    ...
    describe('handleLeaveLobby', () => {
      it('should successfully leave a lobby', async () => {
        mockLobbyService.leaveLobby.mockResolvedValue(mockLobby);
  ...
  describe('Question Set Management', () => {
    describe('handleUpdateQuestionSets', () => {
      it('should successfully update question sets', async () => {
        ...
        expect(socketService.broadcastToLobby).toHaveBeenCalledWith('ABC123', 'question-sets-updated', {
          lobby: mockLobby,
          updatedBy: '1'
        });
      });
  ...
  describe('Public Methods', () => {
    ...
    it('should broadcast to lobby correctly', () => {
      ...
      mockIo.to = jest.fn().mockReturnValue(mockToResult);
      
      socketService.broadcastToLobby('ABC123', 'test-event', { data: 'test' });
      
      expect(mockIo.to).toHaveBeenCalledWith('ABC123');
      expect(mockToResult.emit).toHaveBeenCalledWith('test-event', { data: 'test' });
    });
