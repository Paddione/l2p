...
export class FileProcessingService {
  ...
  private async extractPdfContent(
    ...
    try {
      // Dynamic import to avoid loading test files during module initialization
      ...
      if (data.info) {
        ...
        if (data.info.CreationDate) {
          const creationDate = this.parsePdfDate(data.info.CreationDate);
          if (creationDate) metadata.creationDate = creationDate;
        }
        if (data.info.ModDate) {
          const modDate = this.parsePdfDate(data.info.ModDate);
