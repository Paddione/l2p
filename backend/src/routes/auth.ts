...
import { authenticate, validateRefreshToken, AuthenticatedRequest } from '../middleware/auth.js';
...
router.get('/me', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    const user = await authService.getUserByToken(req.cookies?.accessToken || req.headers.authorization?.substring(7) || '');
