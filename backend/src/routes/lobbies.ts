...
router.get('/:code', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(500).json({
      error: 'Failed to retrieve lobby',
      message: 'An unexpected error occurred while retrieving the lobby'
    });
  }
}));
...
router.post('/:code/start', authenticate, asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message === 'All players must be ready before starting the game') {
        res.status(409).json({
      ...
      if (error.message === 'At least one player is required to start the game') {
        res.status(409).json({
...
router.delete('/cleanup', authenticate, asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  const hoursOld = req.query.hours ? parseInt(req.query.hours as string) : 24;
 ...
 */
router.get('/question-sets/available', authenticate, asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  if (!req.user) {
    res.status(401).json({
      error: 'Authentication required',
      message: 'User must be authenticated to view question sets'
    });
    return;
  }
