  ...
  try {
    ...
    res.json({
      ...
      message: result.levelUp 
        ...
        : `Experience awarded successfully`
    });
  } catch (error) {
 ...
 * Calculate experience requirements for levels (for UI display)
 */
router.get('/experience/calculate', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    const { level } = req.query;
    const targetLevel = level ? parseInt(level as string) : 10;
