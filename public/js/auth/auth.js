...
export function initAuth(storage, screenManager) {
    ...
    async function handleAuthSubmit(e) {
        ...
        try {
            ...
            if (!response.user) {
                throw new Error(`${type === 'login' ? 'Login' : 'Registration'} failed: Invalid response from server`);
            }
    ...
    function switchAuthMode() {
        ...
        if (isRegisterMode) {
            ...
            if (formTitle) formTitle.textContent = 'Welcome to Quiz Game';
            if (submitBtn) submitBtn.textContent = 'Sign In';
            if (switchLink) switchLink.textContent = "Don't have an account? Sign Up";
