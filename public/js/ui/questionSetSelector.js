...
export function initQuestionSetSelector(questionSetsApi, screenManager, lobbyManager, storage, playerManager = null) {
    ...
    function createQuestionSetCard(questionSet) {
        ...
        card.className = 'question-set-card clickable';
    ...
    function handleQuestionCountChange() {
        ...
        if (questionCountInput.value === '' || isNaN(value)) {
            ...
            // Auto-create game when valid question count is entered
            setTimeout(() => {
                handleCreateGameWithQuestionSet();
            }, 500);
        }
    ...
    async function handleCreateGameWithQuestionSet() {
        ...
        try {
            ...
            if (lobbyData) {
                ...
                if (playerManager) {
                    if (typeof playerManager.setCurrentPlayer === 'function') {
                        console.log('Setting current player in playerManager:', currentUser);
                        playerManager.setCurrentPlayer(currentUser);
                    }
                    
                    if (typeof playerManager.setCurrentLobby === 'function') {
                        console.log('Setting current lobby in playerManager:', lobbyData);
                        playerManager.setCurrentLobby(lobbyData);
                    }
                }
                ...
                document.dispatchEvent(lobbyCreatedEvent);
                ...
                // Add a small delay to ensure playerManager processes the lobby data
                setTimeout(() => {
