...
export function initHallOfFameUI() {
    ...
    async function setupUI() {
        ...
        container.innerHTML = `
            <div class="hall-of-fame-header">
                <h2>🏆 Hall of Fame</h2>
                <p class="subtitle">Choose a questionset to view the top 10 scores</p>
            </div>
            
            <div class="catalog-selection">
                <label for="catalog-select">Choose questionset:</label>
                <select id="catalog-select" class="catalog-selector">
                    <option value="">Choose a questionset...</option>
            ...
            <div class="hall-of-fame-content">
                <div class="leaderboard-section">
                    <div id="leaderboard-container" class="leaderboard-container">
                        ${currentCatalog ? '' : '<div class="no-selection">Please select a questionset to view scores</div>'}
                    </div>
                </div>
            </div>
            
            <div class="hall-of-fame-actions">
                <button id="back-from-hof" class="btn-secondary">Back to Menu</button>
            </div>
        `;
    ...
    function showNoSelection() {
        ...
        if (container) {
            container.innerHTML = '<div class="no-selection">Please select a questionset to view scores</div>';
        }
    }
    ...
    async function refreshLeaderboard() {
        ...
        try {
            ...
            container.innerHTML = '<div class="error">Failed to load scores. Please try again.</div>';
        }
    }
    ...
    function updateLeaderboardUI(entries) {
        ...
        if (!entries || entries.length === 0) {
            container.innerHTML = `
                <div class="no-scores">
                    ...
                    <h3>No Scores Yet</h3>
                    <p>Be the first to submit a score for <strong>${currentCatalog}</strong>!</p>
                </div>
            `;
            return;
        }
        ...
        const leaderboardHTML = `
            <div class="leaderboard-header">
                <h3>Top ${entries.length} Score${entries.length !== 1 ? 's' : ''} for ${currentCatalog}</h3>
            </div>
                        ...
                        <div class="leaderboard-entry ${isTopThree ? 'top-three' : ''} rank-${rank}">
                            ...
                            <div class="score-section">
                                ...
                                <div class="score-details">
                                    <span class="accuracy">${Math.round(entry.accuracy || 0)}% accuracy</span>
