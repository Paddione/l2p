...
async function refreshActiveLobbies() {
    ...
    try {
        ...
        if (lobbies.length === 0) {
            activeLobbiesList.innerHTML = '<div class="no-lobbies">' + t('STATUS.NO_ACTIVE_GAMES_FOUND') + '</div>';
            return;
        }

        activeLobbiesList.innerHTML = lobbies.map(lobby => `
        ...
        activeLobbiesList.innerHTML = '<div class="error">' + t('STATUS.FAILED_TO_LOAD_ACTIVE_GAMES') + '</div>';
        showNotification(t('ERRORS.FAILED_TO_JOIN_GAME') + ': ' + error.message, 'error');
    }
}
