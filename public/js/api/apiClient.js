...
const MAX_RETRIES = 2; // Reduced from 3 to 2 to avoid excessive retries
const RETRY_DELAY = 1000; // 1 second
const RETRY_STATUS_CODES = [502, 503, 504]; // Removed 429 from auto-retry codes
const RATE_LIMIT_RETRY_CODES = [429]; // Special handling for rate limiting
...
class ApiClient {
    ...
    async fetchWithTimeout(resource, options, timeout = REQUEST_TIMEOUT, retryCount = 0) {
        ...
        try {
            ...
            // Handle rate limiting separately with more conservative retries
            if (RATE_LIMIT_RETRY_CODES.includes(response.status) && retryCount < 1) {
                // Only retry once for rate limiting, with longer delay
                const retryDelay = Math.min(15000, 5000 * (retryCount + 1)); // 5-15 seconds
                console.warn(`Rate limited (429), attempting single retry after ${retryDelay}ms`);
                await this.sleep(retryDelay);
                return this.fetchWithTimeout(resource, options, timeout, retryCount + 1);
            }
            
            // If we get other retryable status codes and haven't exceeded max retries
            if (RETRY_STATUS_CODES.includes(response.status) && retryCount < MAX_RETRIES) {
                const retryDelay = RETRY_DELAY * (retryCount + 1); // Linear backoff
            ...
            if (error.name === 'AbortError') {
                // If timeout and we haven't exceeded max retries (reduced retries for timeout)
                if (retryCount < 1) {
                    console.warn(`Request timed out, attempting single retry`);
                    await this.sleep(RETRY_DELAY * 2); // Wait longer before timeout retry
                    return this.fetchWithTimeout(resource, options, timeout, retryCount + 1);
                }
                throw new Error(`Request timed out after retry attempts`);
            }
            throw error;
        }
    }
    ...
    async request(method, endpoint, body = null, customHeaders = {}, isFormData = false) {
        ...
        // Only log non-GET requests or when debugging is enabled
        if (method !== 'GET' || window.DEBUG_API) {
                ...
                if (response.status === 401) {
                    ...
                    try {
                        if (window.appState && window.appState.modules && window.appState.modules.screenManager) {
                            window.appState.modules.screenManager.showScreen('auth-screen');
                        } else {
    ...
    async post(endpoint, body = null, headers = {}, isFormData = false) {
        return this.request('POST', endpoint, body, headers, isFormData);
    }

    async put(endpoint, body = null, headers = {}, isFormData = false) {
        return this.request('PUT', endpoint, body, headers, isFormData);
    }
    ...
    async returnToLobby(code) {
        return this.request('POST', `/lobbies/${code}/return-to-lobby`);
    }

    // Rejoin lobby after host has returned (other players)
    async rejoinLobby(code) {
        return this.request('POST', `/lobbies/${code}/rejoin-lobby`);
    }
