...
export function initGameController(lobbyManager, playerManager, storage, screenManager) {
    ...
    async function handleGameStarted(event) {
        try {
            ...
            showNotification(t('ERRORS.FAILED_TO_START_GAME') + ': ' + error.message, 'error');
    ...
    function handleQuestionStarted(event) {
        ...
        // Reset answer submission tracking for new question
        hasSubmittedAnswerForCurrentQuestion = false;
    ...
    function handleGameEnded(event) {
        ...
        const isHost = currentLobby && currentLobby.host === currentUser.username;
        
        // Get additional game data for Hall of Fame and lobby management
        const gameData = {
            ...
            lobbyCode: currentLobby ? currentLobby.code : null,
            isHost: isHost,
            gamePhase: currentLobby ? currentLobby.game_phase : 'finished',
            // Check if all questions were played (for Hall of Fame eligibility)
            playedAllQuestions: currentLobby && currentLobby.question_set && 
                               finalResults.totalQuestions === currentLobby.question_set.question_count
        };
    ...
    function getCurrentCatalogName() {
        ...
        const storedQuestionSet = localStorage.getItem('learn2play_selected_question_set');
    ...
    function updateGameUIElements(questionData) {
        ...
        if (answersContainer) {
            ...
            if (questionData.type === 'multiple_choice' && questionData.options) {
                ...
                answersContainer.innerHTML = '<div class="error">' + t('STATUS.ERROR_LOADING_QUESTION_OPTIONS') + '</div>';
                return;
            }
        } else {
    ...
    function updatePlayerList(questionData) {
        ...
        if (Array.isArray(questionData.players)) {
            ...
            questionData.players.forEach(player => {
                ...
                const multiplier = questionData.multipliers?.[player.username] || 1;
                playerCard.innerHTML = `
                    <div class="player-avatar-container">
                        <div class="player-avatar">${player.character}</div>
                        <div class="multiplier-badge" data-value="${multiplier}">x${multiplier}</div>
                    </div>
            ...
            Object.entries(questionData.players).forEach(([username, playerData]) => {
                const playerCard = document.createElement('div');
                playerCard.className = 'player-card';
                playerCard.setAttribute('data-player', username);
                const multiplier = questionData.multipliers?.[username] || 1;
                playerCard.innerHTML = `
                    <div class="player-avatar-container">
    ...
    // Track answer submission to prevent duplicates
    let answerSubmissionInProgress = false;
    let hasSubmittedAnswerForCurrentQuestion = false;
    ...
    async function handleAnswerClick(answer, buttonElement) {
        try {
            // Prevent multiple clicks and duplicate submissions
            if (buttonElement.disabled || answerSubmissionInProgress || hasSubmittedAnswerForCurrentQuestion) {
                console.log('Answer submission blocked: disabled, in progress, or already submitted');
                return;
            }
            ...
            // Mark as submitted for current question
            hasSubmittedAnswerForCurrentQuestion = true;
            ...
            // Only re-enable buttons if it's not an "already answered" error
            ...
            if (error.message.includes('Already answered')) {
                ...
                showNotification(t('ERRORS.FAILED_TO_SUBMIT_ANSWER') + ': ' + error.message, 'error');
            }
        } finally {
    ...
    function showQuestionResults(results) {
        ...
        // Update all player scores and multipliers in the UI with animations
        if (results.scores) {
            updateAllPlayerScores(results.scores, results.multipliers);
        }
    }
     ...
     * @param {Object} multipliers - Player multipliers object (optional)
     */
    function updateAllPlayerScores(scores, multipliers = null) {
        const playerCards = document.querySelectorAll('#game-players-list .player-card');
        playerCards.forEach(card => {
            const nameElement = card.querySelector('.player-name');
            const scoreElement = card.querySelector('.player-score');
            const multiplierElement = card.querySelector('.multiplier-badge');
            ...
            if (nameElement && scoreElement) {
                ...
                if (multipliers && multipliers[username] !== undefined && multiplierElement) {
                    const oldMultiplier = parseInt(multiplierElement.dataset.value) || 1;
                    const newMultiplier = multipliers[username];
                    
                    if (newMultiplier !== oldMultiplier) {
                        multiplierElement.dataset.value = newMultiplier;
                        multiplierElement.textContent = `x${newMultiplier}`;
