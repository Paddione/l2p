...
export function initGameEngine(lobbyManager, questionManager, storage) {
    ...
    function startGameStatePolling() {
        ...
        // Start with optimized polling interval
        let currentInterval = PERFORMANCE_SETTINGS.POLLING_INTERVALS.GAME_STATE_MIN;
            ...
            gameStateInterval = setTimeout(async () => {
                try {
                    if (currentGame && currentGame.lobbyCode) {
                        ...
                        // If successful, gradually reduce interval back to normal
                        if (rateLimitBackoffCount > 0) {
                            rateLimitBackoffCount--;
                            currentInterval = Math.max(
                                PERFORMANCE_SETTINGS.POLLING_INTERVALS.GAME_STATE_MIN, 
                                PERFORMANCE_SETTINGS.POLLING_INTERVALS.GAME_STATE_MIN + (rateLimitBackoffCount * 1000)
                            );
                    ...
                    if (error.message && error.message.includes('Too Many Requests') || error.message.includes('429')) {
                        ...
                        currentInterval = Math.min(
                            PERFORMANCE_SETTINGS.POLLING_INTERVALS.GAME_STATE_MAX,
                            PERFORMANCE_SETTINGS.POLLING_INTERVALS.GAME_STATE_MIN + (rateLimitBackoffCount * 2000)
                        );
