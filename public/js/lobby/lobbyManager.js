...
export function initLobbyManager(storage, screenManager) {
    ...
    function areAllPlayersReady(lobby) {
        ...
        // Hosts are considered always ready
    ...
    /**
     * Sets the question count for a lobby via API
     * @param {string} code - Lobby code
     * @param {number} questionCount - Number of questions to use
     * @returns {Promise<Object>} - Updated lobby data
     */
    async function setQuestionCount(code, questionCount) {
        try {
            return await apiClient.request('POST', `/lobbies/${code}/question-count`, {
                question_count: questionCount
            });
        } catch (error) {
            console.error('Failed to set question count:', error);
            throw new Error(error.message || 'Failed to set question count');
        }
    }
