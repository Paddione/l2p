...
// Mock the API service functions first (before jest.mock calls)
  ...
  describe('Reset Password Step', () => {
    ...
    it('validates password requirements in real-time', async () => {
      ...
      await waitFor(() => {
        ...
        expect(requirements[1]).toHaveClass('invalid') // lowercase
    ...
    it('submits reset form successfully with valid password', async () => {
      ...
      await user.click(submitButton)
      await waitFor(() => {
        expect(mockCompletePasswordReset).toHaveBeenCalledWith('test@example.com', 'reset-token-123', 'StrongPass123!')
    ...
    it('shows loading state during reset', async () => {
      mockCompletePasswordReset.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)))
    ...
    it('calls onBackToLogin after successful reset', async () => {
      ...
      // Wait for the timeout to trigger onBackToLogin
      await waitFor(() => {
