    ...
    it('handles streak edge cases for correct answers', () => {
      ...
      extremeStreaks.forEach(streak => {
        ...
        expect(mockAudioStore.playCorrectAnswer).toHaveBeenCalledWith(streak)
         ...
         it('handles background music with various loop values', () => {
       const { result } = renderHook(() => useAudio())
       
       // Test different loop values
       act(() => {
       ...
       act(() => {
         result.current.handleBackgroundMusic(false)
       })
       expect(mockAudioStore.playBackgroundMusic).toHaveBeenCalledWith(false)
       
       act(() => {
         result.current.handleBackgroundMusic()
       })
       expect(mockAudioStore.playBackgroundMusic).toHaveBeenCalledWith(true) // Default to true
     })
  })
