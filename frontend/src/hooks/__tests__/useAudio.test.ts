...
describe('useAudio Hook', () => {
  ...
  describe('Background Music Handlers', () => {
    it('provides background music handlers', () => {
      const { result } = renderHook(() => useAudio())
      
      expect(typeof result.current.handleBackgroundMusic).toBe('function')
      expect(typeof result.current.handleLobbyMusic).toBe('function')
    })

    it('calls appropriate store methods for background music', () => {
      const { result } = renderHook(() => useAudio())
      
      act(() => {
        result.current.handleBackgroundMusic(true)
        result.current.handleLobbyMusic(false)
      })
      
      expect(mockAudioStore.playBackgroundMusic).toHaveBeenCalledWith(true)
      expect(mockAudioStore.playLobbyMusic).toHaveBeenCalledWith(false)
    })

    it('uses default loop parameter when not specified', () => {
      const { result } = renderHook(() => useAudio())
      
      act(() => {
        result.current.handleBackgroundMusic()
        result.current.handleLobbyMusic()
      })
      
      expect(mockAudioStore.playBackgroundMusic).toHaveBeenCalledWith(true)
      expect(mockAudioStore.playLobbyMusic).toHaveBeenCalledWith(true)
    })
  })
  ...
  describe('Integration with Audio Store', () => {
    it('passes through all store state correctly', () => {
      ...
      mockAudioStore.currentTrack = 'background-music' as any
      ...
      expect(result.current.currentTrack).toBe('background-music')
    })
