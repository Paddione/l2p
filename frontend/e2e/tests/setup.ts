...
export class TestHelpers {
  static async registerUser(page: any, username?: string, email?: string, password?: string) {
    const timestamp = Date.now();
  ...
  static async createLobby(page: any, questionCount: string = '5', questionSet: string = 'general') {
    await page.click('[data-testid="create-lobby-button"]');
  ...
  static async joinLobby(page: any, lobbyCode: string) {
  ...
  static async waitForGameStart(page: any) {
    await expect(page.locator('[data-testid="question-container"]')).toBeVisible({ timeout: 15000 });
  }

  static async answerQuestion(page: any, optionIndex: number = 0) {
  ...
  static async simulateNetworkError(page: any, pattern: string = '**/api/**') {
    await page.route(pattern, route => route.abort('failed'));
  }

  static async simulateSlowNetwork(page: any, delay: number = 5000) {
  ...
  static async mockApiResponse(page: any, url: string, response: any, status: number = 200) {
  ...
  static async simulateUserInactivity(page: any, duration: number = 60000) {
